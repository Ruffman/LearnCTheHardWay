...
static char* test_operations()
{
	...
	mu_assert(make_random(map), "Didn't make a random fake radix max");
	
	RadixMap_sort(map);
	mu_assert(check_order(map), "Failed to properly sort the radix map");
	mu_assert(test_search(map), "Failed the search test");
	mu_assert(check_order(map), "RadixMap didn't stay ordered after search");
	...
	while (map->end > 0) {
		...
		mu_assert(check_order(map), "RadixMap didn't stay sorted after deletion");
	}
